...
public class EvaluationRepository {
    ...
    public List<Respuesta> getRespuestas() {
        ...
        for(Respuesta respuesta : respuestas) {
            ...
            // Pregunta #46
            evaluados = evaluatedsPorPregunta.get(45);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta46(evaluadted.getCorrecta());
                }
            }

            // Pregunta #47
            evaluados = evaluatedsPorPregunta.get(46);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta47(evaluadted.getCorrecta());
                }
            }

            // Pregunta #48
            evaluados = evaluatedsPorPregunta.get(47);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta48(evaluadted.getCorrecta());
                }
            }

            // Pregunta #49
            evaluados = evaluatedsPorPregunta.get(48);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta49(evaluadted.getCorrecta());
                }
            }

            // Pregunta #50
            evaluados = evaluatedsPorPregunta.get(49);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta50(evaluadted.getCorrecta());
                }
            }

            // Pregunta #51
            evaluados = evaluatedsPorPregunta.get(50);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta51(evaluadted.getCorrecta());
                }
            }

            // Pregunta #52
            evaluados = evaluatedsPorPregunta.get(51);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta52(evaluadted.getCorrecta());
                }
            }

            // Pregunta #53
            evaluados = evaluatedsPorPregunta.get(52);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta53(evaluadted.getCorrecta());
                }
            }

            // Pregunta #54
            evaluados = evaluatedsPorPregunta.get(53);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta54(evaluadted.getCorrecta());
                }
            }

            // Pregunta #55
            evaluados = evaluatedsPorPregunta.get(54);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta55(evaluadted.getCorrecta());
                }
            }

            // Pregunta #56
            evaluados = evaluatedsPorPregunta.get(55);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta56(evaluadted.getCorrecta());
                }
            }

            // Pregunta #57
            evaluados = evaluatedsPorPregunta.get(56);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta57(evaluadted.getCorrecta());
                }
            }

            // Pregunta #58
            evaluados = evaluatedsPorPregunta.get(57);
            for (Evaluated evaluadted: evaluados) {
                if(evaluadted.getIdEvaluated() == respuesta.getEvaluadoid()) {
                    respuesta.setPregunta58(evaluadted.getCorrecta());
                }
            }
